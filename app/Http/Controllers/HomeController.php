...
class HomeController extends Controller
{
    ...
    public function studentDetails(Request $request, $id) {
        try{    
            ...
            $course = $course->where('uc.user_id', $id)->where('is_expire', 0)->select('c.course_fee', 'c.id', 'uc.status', 'uc.created_date', 'uc.updated_date', 'uc.buy_price', 'c.title', 'c.valid_upto', 'c.introduction_image', DB::raw('(select COUNT(*) FROM course_chapter WHERE course_chapter.course_id = uc.course_id) as chapter_count'), DB::raw("(SELECT orders.id FROM orders INNER JOIN order_product_detail ON orders.id = order_product_detail.order_id WHERE orders.user_id = $id AND orders.status = 1 AND order_product_detail.product_id = c.id AND order_product_detail.product_type = 1) as order_id"))->where('c.admin_id', auth()->user()->id)->orderByDesc('uc.id')->distinct('uc.id')->paginate(3);
